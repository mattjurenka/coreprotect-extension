  ...
  <div class="flex mb-2">
    <div class="flex flex-col justify-center">
      <p class="text-base font-jetbrains underline decoration-2 underline-offset-2 font-bold">
        ERC20 Mint - <Etherscanlink contract_hex={contract}/>
      </p>
    </div>
  ...
  {#if help_expanded}
    <div class="my-1 p-2 bg-lightgrey">
      <p class="font-jetbrains text-paragraph">
        This transaction includes a mint (token generation) of {format_decimal(tokens_transferred)} coins of <Etherscanlink contract_hex={contract}/>
        given to <Etherscanlink contract_hex={account} />. At current Uniswap price, this mint represents a value of ${format_decimal(dollars_transferred)} in USD.
      </p>
    </div>
  {/if}
